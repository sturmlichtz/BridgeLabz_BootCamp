...
if [ $isOne -gt $isTwo -a $isOne -gt $isThree -a $isOne -gt $isFour -a $isOne -gt $isFive -a $isOne -gt $isSix ]
then
	echo "Dice 1 reached for maximum" $isOne "times."
elif [ $isTwo -gt $isOne -a $isTwo -gt $isThree -a $isTwo -gt $isFour -a $isTwo -gt $isFive -a $isTwo -gt $isSix ]
then
	echo "Dice 2 reached for maximum" $isTwo "times."
elif [ $isThree -gt $isTwo -a $isThree -gt $isOne -a $isThree -gt $isFour -a $isThree -gt $isFive -a $isThree -gt $isSix ]
then
	echo "Dice 3 reached for maximum" $isThree "times."
elif [ $isFour -gt $isTwo -a $isFour -gt $isThree -a $isFour -gt $isOne -a $isFour -gt $isFive -a $isFive -gt $isSix ]
then
